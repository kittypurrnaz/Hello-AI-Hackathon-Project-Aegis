...
// // This will only allow requests from the frontend URL you set in Cloud Run
// // const allowedOrigins = [process.env.FRONTEND_URL, 'http://localhost:3000', 'https://aegis-frontend-service-943089436637.us-central1.run.app/'];
// // const corsOptions = {
...
// app.use(cors()); 
// // --- FIX: This line is crucial. It allows the server to parse incoming JSON requests. ---
...
//     `;
//     const options = { query: query, location: 'US' }; // Adjust location if needed
...
// // --- ENDPOINT TO GET DASHBOARD DATA FOR A USER ---
...
//     const options = {
//         query: query,
//         location: 'US', // Adjust location if needed
...
//         console.log(`Successfully fetched ${rows.length} rows for user ${userId}.`);
//         res.status(200).json(rows);
//     } catch (error) {
...
// // --- GEMINI FEATURE: UPDATED FOR STRUCTURED JSON OUTPUT ---
...
//     // STRUCTURED PROMPT: Asking for a JSON object
...
//             // Add generationConfig to request a JSON response
//             generationConfig: {
...
//         // The summary is now a JSON string, so we parse it
...
// // --- CORRECTED ENDPOINT TO GET DASHBOARD DATA FOR A USER ---
// app.get('/api/dashboard-data/:userId', async (req, res) => {
//     const userId = req.params.userId;
...
//     console.log(`Fetching data for user: ${userId} with date range ${startDate} to ${endDate}.`);
...
//         // BigQuery's `DATE` function is reliable for date filtering
//         query += ` AND DATE(timestamp) BETWEEN DATE(@startDate) AND DATE(@endDate)`;
//         options.params.startDate = startDate; // Already formatted as 'YYYY-MM-DD' from frontend
//         options.params.endDate = endDate;
//     }

//     query += ` ORDER BY timestamp DESC`; // Removed LIMIT 100
...
//         res.status(200).json({ summary: summaryJson });
//     } catch (error) {
//         console.oerror('Error calling Gemini API:', error);
//         res.status(500).json({ error: `Failed to generate summary. ${error.message}` });
//     }
// });
...
// // --- MAIN ENDPOINT FOR DASHBOARD DATA (No change here, as it's meant to be called by the frontend) ---
// app.get('/api/dashboard-data/:userId', async (req, res) => {
//     const userId = req.params.userId;
//     const { startDate, endDate } = req.query;
//     if (!userId) {
...
// // --- NEW TEST ENDPOINT FOR CONSOLE LOGGING ---
// app.get('/api/test-data/:userId/:timePeriod', async (req, res) => {
//     const userId = req.params.userId;
...
//         endDate = now.toISOString().split('T')[0];
//     } else {
//         return res.status(400).send({ message: 'Invalid time period. Use "today", "7", or "30".' });
//     }

//     console.log(`--- Test Endpoint Called ---`);
//     console.log(`Simulating request for user: ${userId} with period: ${timePeriod}`);
...
//         // --- NEW LOGGING: DISPLAYING THE RAW DATA AND COUNTS ---
//         const neutralCount = rows.filter(row => row.signal_type?.includes('NEUTRAL')).length;
//         const immediateCount = rows.filter(row => row.signal_type?.includes('IMMEDIATE')).length;
//         const intermediateCount = rows.filter(row => row.signal_type?.includes('INTERMEDIATE')).length;

//         console.log(`Total rows fetched from BQ: ${rows.length}`);
//         console.log(`Flag counts from BQ: Neutral=${neutralCount}, Intermediate=${intermediateCount}, Immediate=${immediateCount}`);
        
//         console.log('--- Sample Raw Data (first 5 rows) ---');
...
//         res.status(200).json({
//             message: 'Test data logged to console.',
//             counts: { neutralCount, intermediateCount, immediateCount },
//             totalRows: rows.length,
//         });

//     } catch (error) {
//         console.error('BIGQUERY ERROR fetching test data:', error);
//         res.status(500).send({ message: 'Error fetching test data from BigQuery', error: error.message });
//     }
// });
