...
//   const [browsingHistory, setBrowsingHistory] = useState<BrowsingSession[]>([]);
...
//           const profiles = users.map((user: any, index: number) => ({
...
//   // 2. Fetch dashboard data when the active child changes
//   useEffect(() => {
...
//         setError(null);
//         setIsLoading(true);
//         const response = await fetch(`${API_BASE_URL}/dashboard-data/${activeChild}`);
...
//   useEffect(() => {
//   // When the modal opens, start generating the report.
//   if (isReportModalOpen) {
//     runAdkReportGeneration();
//   }
// }, [isReportModalOpen]);
...
// const sendPrompt = async (promptText: string, onChunk: (chunk: string) => void) => {
...
//   // 3. Helper function to transform raw BQ data into formats the UI needs
//   const processBigQueryData = (data: any[]) => {
//     // Populate the detailed activity log data
//   const logData: ActivityLogEntry[] = data.map((row: any) => ({
...
//   }));
//   setActivityLogData(logData);
//     // A. Transform for Browsing History
//     const history: BrowsingSession[] = data.map((row: any) => {
//         // CORRECTED: Check row.signal_type for severity
//         const getStatus = (signal: string): BrowsingSession['status'] => {
//             if (signal?.includes('IMMEDIATE')) return 'blocked';
//             if (signal?.includes('INTERMEDIATE')) return 'flagged';
//             return 'allowed'; 
//         };

//         // Map BQ `topic_category` to UI `category`
//         const getCategory = (topic: string): BrowsingSession['category'] => {
//             const validCategories = ['educational', 'entertainment', 'social', 'gaming'];
//             return validCategories.includes(topic) ? topic as BrowsingSession['category'] : 'other';
//         };

//         return {
//             id: row.timestamp?.value || Math.random().toString(),
//             website: row.source_platform || 'Unknown Website',
//             domain: row.event_details?.url || 'N/A',
//             timeSpent: 5, 
//             timestamp: row.timestamp ? new Date(row.timestamp.value) : new Date(),
//             category: getCategory(row.topic_category),
//             // CORRECTED: Pass signal_type to the function
//             status: getStatus(row.signal_type),
//         };
//     });
//     setBrowsingHistory(history);

//     // B. Transform for Threat Flags
//     const flags = {
//         neutral: { count: 0, items: [] as any[] },
//         intermediate: { count: 0, items: [] as any[] },
//         immediate: { count: 0, items: [] as any[] }
//     };

//     data.forEach(row => {
//         const itemDetail = {
//             // Use flag_type for the description, as it's more specific
...
//         // CORRECTED: Check row.signal_type instead of row.flag_type
...
//     const threatFlagData: ThreatFlag[] = [
...
//     ];
//     setThreatFlags(threatFlagData);

//     // C. Transform for Weekly Usage (This also uses the corrected logic now)
//     const usageByDay: { [key: string]: DailyUsage } = {};
//     const days = ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'];
//     days.forEach(day => {
//         usageByDay[day] = { date: day, screenTime: 0, sitesVisited: 0, sitesBlocked: 0, neutral: 0, intermediate: 0, immediate: 0 };
//     });

//     data.forEach(row => {
//         const day = days[new Date(row.timestamp.value).getDay()];
//         usageByDay[day].sitesVisited++;
//         usageByDay[day].screenTime += 5;
//         // CORRECTED: Check row.signal_type for counting
...
//     // D. Fetch AI-powered summary from Gemini
//     fetchAegisReport(data);
//   };


//   // 4. Function to fetch summary from the Gemini endpoint
//   const fetchAegisReport = async (activityData: any[]) => {
...
//     // Note: The old status and category filters are removed as they don't apply to the new design
...
//     // 3. Find the top concern for each platform
...
//             platform,
//             topConcern,
//             count: flagCounts[topConcern],
//             totalVisits: data.totalVisits,
//         };
//     });

//     // 4. Sort by the total number of concerning visits
//     return platformList.sort((a, b) => b.totalVisits - a.totalVisits);

// }, [activityLogData]);

// // --- New hook to calculate the highest risk category and its avg. confidence ---
...
//     // 1. Determine the highest severity level present in the data
...
//     // 2. Filter for events that match that highest level
//     const highestRiskEvents = activityLogData.filter(log => 
//         log.signal_type?.includes(highestRiskLevel.toUpperCase())
//     );

//     // 3. Find the most frequent specific flag_type within those events
...
//     // 4. Calculate the average confidence for ONLY that top risk
...
//   // --- UI HELPER FUNCTIONS & HANDLERS (Mostly unchanged) ---
...
//     return;
//   }
//   setStreamedReportContent(''); // Clear old report content
//   setIsReportModalOpen(true);  // This opens the pop-up
// };

// // This new function contains the streaming API logic
// const runAdkReportGeneration = async () => {
//   setIsGeneratingReport(true);
//   const prompt = `Generate a digital well-being report for child '${activeChild}' for the last 7 days, including a summary, key insights, and recommendations.`;
...
//       if (done) break;
//       const chunk = decoder.decode(value);
//       setStreamedReportContent((prevContent) => prevContent + chunk);
//     }

//   } catch (error) {
//     const errorMessage = `Error generating report: ${error.message}. Please check the ADK server URL and ensure it is running.`;
//     setStreamedReportContent(errorMessage);
//     console.error(error);
//   } finally {
...
// // This new function handles downloading the report text
...
//                   <CardTitle className="text-sm">Today's Estimated Screen Time (Est.)</CardTitle>
//                   <Clock className="h-4 w-4 text-muted-foreground" />
//               </CardHeader>
//               <CardContent>
//                   <div className="text-2xl">{weeklyUsage.reduce((acc, day) => acc + day.screenTime, 0)}m</div>
//                   <p className="text-xs text-muted-foreground">Based on events logged</p>
//               </CardContent>
//           </Card>

//           {/* NEW: Total Events Card */}
//           <Card>
//               <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
...
//                                   {aegisReport.keyInsights.map((insight: string, index: number) => (
...
//                                 // Helper to get border color based on signal type
...
//       <CardHeader>
//         <CardTitle className="flex items-center gap-2"><MessageCircle className="w-5 h-5" />Chat with Aegis Agent</CardTitle>
...
//                 <div className={`max-w-[80%] p-3 rounded-lg ${chat.sender === 'user' ? 'bg-primary text-primary-foreground' : 'bg-muted'}`}>
//                   {chat.message === '' && chat.sender === 'agent' ? <Loader2 className="w-4 h-4 animate-spin" /> : <p className="text-sm leading-relaxed">{chat.message}</p>}
//                 </div>
//               </div>
//             ))}
//           </div>
//         </ScrollArea>
//         <div className="flex gap-2">
//           <Input placeholder="Ask a question..." value={chatMessage} onChange={(e) => setChatMessage(e.target.value)} onKeyPress={(e) => e.key === 'Enter' && handleChat()} disabled={isAgentLoading}/>
//           <Button onClick={handleChat} disabled={isAgentLoading || !chatMessage.trim()} size="sm"><Send className="w-4 h-4" /></Button>
//         </div>
//       </CardContent>
//     </Card>
//   </div>
...
//                     <AlertDescription>
//                         Profile management is disabled as profiles are loaded from your database.
//                     </AlertDescription>
//                 </Alert>
